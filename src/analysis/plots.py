...
def violin_by_strategy(df: pd.DataFrame, metric: str, density: str, out_path: Path, ylabel: str = None):
    ...
    strategies = _ordered_strategies(subset)
    data = [
        subset[subset["strategy"] == s][metric].replace([np.inf, -np.inf], np.nan).dropna().to_numpy()
        for s in strategies
    ]
    fig, ax = plt.subplots(figsize=(6, 4))
    ax.violinplot([d for d in data if d.size > 0], showmeans=True)
    ax.set_xticks(range(1, len(strategies) + 1))
    ax.set_xticklabels(strategies, rotation=20)
    ax.set_ylabel(ylabel or metric)
    ax.set_title(f"{metric} by strategy — density={density}")
    fig.tight_layout()
